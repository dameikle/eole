...
class TestAudioPredictorVADModes(unittest.TestCase):
    ...
    def test_predict_both_outputs_segments_words_and_meta(self):
        predictor = AudioPredictor.__new__(AudioPredictor)
        predictor.data_type = "audio"
        predictor.model = _DummyModel()
        predictor.vad_mode = "segment"
        ...
        predictor.language = "en"
        predictor.word_alignment_model = None
        predictor.word_timestamp_heads = None
        predictor.sample_rate = 16000
        predictor.verbose = False
        predictor.report_score = False
        predictor.report_time = False

        segments = [{"start": 0.0, "end": 1.0, "text": "hello", "score": -0.5}]
        words = [{"text": "hello", "start": 0.0, "end": 1.0}]
        predictor._predict_vad_segments = lambda waveform, device, speech_segments, word_backend=None: (segments, words)
        ...
        scores, estim, predictions = predictor._predict(iter(infer_iter))
        payload = json.loads(predictions[0][0])

        self.assertEqual(scores, [[-0.5]])
        self.assertEqual(estim, [[1.0]])
        self.assertEqual(payload["segments"], segments)
        self.assertEqual(payload["words"], words)
        self.assertEqual(payload["meta"]["timestamps"], "both")
        self.assertEqual(payload["meta"]["word_backend"], "wav2vec2")
