...
def clean_example(example):
    ...
    # Waveform examples must not be wrapped in a dict — tensorify_audio
    # expects example["src"] to be a raw tensor.
...
def transform_bucket(task, bucket, threshold=0):
    ...
    for (transform, cid), examples_group in transform_groups.items():
        ...
        for example, _, _ in transformed_group:
            ...
            # Waveform examples skip text-based length check but still respect score threshold
